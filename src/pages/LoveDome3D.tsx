  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-950 via-pink-950 to-purple-900 relative overflow-hidden">
      ...
      <div className="relative z-0" style={{ width: '100vw', height: 'calc(100vh - 200px)' }}>
        {highPerformanceMode ? (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 p-4 h-full overflow-auto">
            {images.map((img, i) => (
              <img key={i} src={img.src} alt={img.alt} className="w-full h-32 object-cover rounded-xl" loading="lazy" />
            ))}
          </div>
        ) : (
          <DomeGallery
