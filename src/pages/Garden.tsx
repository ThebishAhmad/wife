...
const Garden = () => {
  ...
  return (
    <div className="relative w-full h-screen bg-[#2c1810] overflow-hidden font-serif">
      ...
      {highPerformanceMode ? (
        <div className="absolute inset-0 z-10 p-8 overflow-auto flex flex-col items-center">
          <h2 className="text-3xl text-rose-100 font-serif mb-6 text-center">Crystal Memories</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl w-full">
            {memories.map(m => (
              <div key={m.id} className="p-6 rounded-xl border border-rose-300/30 bg-black/30 text-rose-100 font-serif italic text-lg cursor-pointer hover:bg-black/50" onClick={() => handleCrystalClick(m.id)}>
                <Heart className="w-5 h-5 mb-2 text-rose-300" />
                "{m.memory}"
              </div>
            ))}
          </div>
