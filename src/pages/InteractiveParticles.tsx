...
export default function InteractiveParticles() {
  ...
  if (highPerformanceMode) {
    return (
      <div className="w-full h-screen bg-black relative overflow-hidden flex flex-col items-center justify-center text-center p-8">
        <Button onClick={() => navigate(-1)} className="absolute top-4 left-4 z-50">
          <ArrowLeft />
        </Button>
        <Heart className="w-16 h-16 text-pink-500 mb-4" />
        <h2 className="text-3xl font-bold text-white mb-2">Interactive Particles</h2>
        <p className="text-white/60">Turn off performance mode for the hand-tracking particle experience</p>
      </div>
    );
  }
